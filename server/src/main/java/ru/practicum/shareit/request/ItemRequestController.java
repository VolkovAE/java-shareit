...
public class ItemRequestController {
    ...
    @PostMapping
    ItemRequestDto add(@RequestHeader(name = NAME_HEADER_USER_ID, required = true) Long userId,
                       @RequestBody NewRequestItem requestItem) {
        return itemRequestService.add(requestItem, userId);
    }

    @GetMapping
    public Collection<ItemRequestDto> findAllMy(@RequestHeader(name = NAME_HEADER_USER_ID, required = true) Long userId) {
        return itemRequestService.findAllMy(userId);
    }

    @GetMapping("/all")
    public Collection<ItemRequestDto> findAllOther(@RequestHeader(name = NAME_HEADER_USER_ID, required = true) Long userId) {
        return itemRequestService.findAllOther(userId);
    }

    @GetMapping("/{requestId}")
    ItemRequestDto findById(@PathVariable(name = PATH_VARIABLE_REQUEST_ID, required = true) Long requestId) {
